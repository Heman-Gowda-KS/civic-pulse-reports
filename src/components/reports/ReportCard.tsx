...
import { ThumbsUp, ThumbsDown, MapPin, Clock, MessageCircle } from "lucide-react";
...
const ReportCard = ({ report, onVote, isAuthenticated }: ReportCardProps) => {
  ...
  const handleVote = async (voteType: 'up' | 'down') => {
    ...
    try {
      setIsVoting(true);
      console.log('ReportCard: Starting vote for report', report.id, 'with type', voteType);
      
      const result = await voteOnReport(report.id, voteType);
      console.log('ReportCard: Vote result', result);
      
      // Call the parent's onVote to refresh the data
      ...
      console.error('ReportCard: Vote error', error);
      toast({
